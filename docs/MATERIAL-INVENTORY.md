...
|---------|---------|------------------|
| `Material/Flujo_de_Caja_Pyme_SAS completo.xlsx` | Excel | Módulos financieros (libro actual; incluye hoja **Control de Cartera**) |
| `Material/Flujo_de_Caja_Pyme_SAS.xlsx` | Excel | Versión previa / referencia histórica |
...
## Flujo_de_Caja_Pyme_SAS completo.xlsx — hojas

| Hoja Excel | Rutas / tablas app |
|------------|----------------------|
| Supuestos | `scenarios` (nombre, `annual_rate_pct`, `inflation_pct`, notas) |
| Flujo de Caja | `cash_movements` + matriz `cash_flow_sheet_*` (plantilla mensual) |
...
| Control de Cartera | `portfolio_items`: filas CxC con columnas alineadas al Excel (`gross_amount`, `iva_amount`, retenciones, `paid_amount`, `service_description`, `issued_on`, `status`, `notes`, etc.); CxP en la misma tabla con `kind = payable`. UI: `/cartera`. |
